...
if __name__ == '__main__':
    ...
    while True:
        nav = chromeCli.find_element_d(by=By.CSS_SELECTOR, value='[data-testid="level2-navigation-container"]')
        lis = chromeCli.find_elements(src=nav, by=By.CSS_SELECTOR, value='li')
        for (i, li) in enumerate(lis):
            nav = chromeCli.find_element_d(by=By.CSS_SELECTOR, value='[data-testid="level2-navigation-container"]')
            lis = chromeCli.find_elements(src=nav, by=By.CSS_SELECTOR, value='li')
            append(lis[i].get_attribute('innerText'))
            chromeCli.click(lis[i])
            while True:
                grid = chromeCli.find_element_d(by=By.CSS_SELECTOR, value='[data-testid="alaska-grid"]')
                ...
                nextButton = chromeCli.find_element_d(by=By.CSS_SELECTOR, value='[data-testid="pagination-next-button"]')
                if nextButton.get_attribute('disabled') == 'true':
                    break
                chromeCli.click(nextButton)
