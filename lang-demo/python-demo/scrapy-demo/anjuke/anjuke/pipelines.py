    ...
    def process_item(self, item, spider):
        if isinstance(item, CityItem):
            ...
            self.conn.execute("insert into anjuke_new_house(province, city, name, address, type, tag, price, price_num, url, remark) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                              (item.get("province"), item.get("city"), item.get("name"), item.get("address"), item.get("type"), item.get("tag"), item.get("price"), item.get("price_num"), item.get("url"), item.get("remark")))
            self.conn.commit()
